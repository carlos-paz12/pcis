...
void Neander::execute(void (Neander::*f)(uint8_t &end))
{
    ...
    (this->*f)(RI.get_endereco()); ///[!] Chama o método de instância no contexto da classe Neander.
...
{
    AC = ula.sub(AC, memoria.read(end)); ///[!] Subtrai o valor armazenado em memória de AC.
}
