...
int main()
{
    ...
    Memory memory;  // 256 bytes de memória
    Neander processador;

    // Passo 2: Carregar instruções simples na memória.
    // Vamos carregar a memória com as instruções: LDA 5, ADD 5, HLT.
    memory.write(0, Instrucao(Opcode::LDA_END, 5).toUint8());  // Carrega o valor da memória 5 no AC
    memory.write(1, Instrucao(Opcode::ADD_END, 5).toUint8());  // Soma o valor da memória 5 com o AC
    memory.write(2, Instrucao(Opcode::HLT, 0).toUint8());      // Instrução de HALT

    // Passo 3: Definir o valor inicial na memória para teste.
    memory.write(5, 10);  // Definindo o valor na memória endereço 5 como 10

    std::cout << "Memória no endereço 0: " << (int)memory.read(0) << std::endl;  // Verifica o valor no endereço 0

    // Passo 4: Executar o processador.
    processador.run(memory);
